  ...
  "mixed_scenario": {
    ...
    "area_generator": {
      ...
      "area_types": [
        ...
        "highway",                     // RMa_LOS (open rural — most distinct from UMi)
...
### Classification — XGBoost, raw features, temporal split

| Features | h=0 acc | h=1 acc | h=1 MAE |
|---|---|---|---|
| RSS | 18.8% | 23.2% | 5.70m |
| SINR | 18.8% | 22.9% | 5.74m |
| RSS+SINR | 18.8% | 23.0% | 5.72m |
| +AoA azimuth | 69.6% | 77.9% | 0.58m |
| **+AoA elevation** | **84.6%** | **88.5%** | **0.25m** |
| CQI | 1.3% | 1.3% | 9.11m |

**Best: 88.5% accuracy, 0.25m MAE** (XGBoost, rss+sinr+aoa_azimuth+aoa_elevation, h=1)

AoA is the dominant feature (+52pp vs RSS+SINR). RSS ≈ SINR with no interferers. CQI saturates at close range.

Results stored in `results/experiment_matrix/2026-02-26_07-54-19/`

### Regression — RF, rss+sinr+aoa_az+aoa_el

| h | RF Position Error | XGB Position Error |
|---|---|---|
| h=0 | 0.44m | 0.58m |
| h=1 | **0.40m** | 0.59m |

Classification outperforms regression (0.25m vs 0.40m) — discrete grid structure favors classifiers.

---

## Expected Results (Original Predictions)
